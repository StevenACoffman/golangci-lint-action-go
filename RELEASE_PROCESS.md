...
```sh
gowheels pypi --package-name golangci_lint_action_go --name golangci-lint-action-go --repo StevenACoffman/golangci-lint-action-go
```
...
gowheels pypi --name golangci-lint-action-go --package-name golangci_lint_action_go --repo StevenACoffman/golangci-lint-action-go --upload
```
...
```sh
gowheels pypi --name golangci-lint-action-go --package-name golangci_lint_action_go --repo StevenACoffman/golangci-lint-action-go --version v0.5.0 --upload
```
...
3. **Add a trusted publisher** on PyPI:
   ...
   - Fill in the fields:
     ...
     | Repository name  | `golangci-lint-action-go`           |
